...
FAIR_VALUE = 10000
OSMIUM_CONFIG_JSON_ENV = "OSMIUM_CONFIG_JSON"
OSMIUM_CONFIG_PATH_ENV = "OSMIUM_CONFIG_PATH"
...
class OsmiumConfig:
    ...
    reservation_deviation_skew: float = 0.60
    ...
    center_widening_ticks: float = 4.0
    ...
    aggressive_take_threshold: float = 4.0
...
def load_osmium_config_overrides_from_env() -> Dict[str, Any]:
    overrides: Dict[str, Any] = {}

    config_path = os.getenv(OSMIUM_CONFIG_PATH_ENV, "").strip()
    if config_path:
        with open(config_path, "r", encoding="utf-8") as handle:
            loaded = json.load(handle)
        if not isinstance(loaded, dict):
            raise ValueError(f"{OSMIUM_CONFIG_PATH_ENV} must point to a JSON object")
        overrides.update(loaded)

    raw_json = os.getenv(OSMIUM_CONFIG_JSON_ENV, "").strip()
    if raw_json:
        loaded = json.loads(raw_json)
        if not isinstance(loaded, dict):
            raise ValueError(f"{OSMIUM_CONFIG_JSON_ENV} must contain a JSON object")
        overrides.update(loaded)

    return overrides


def load_osmium_config_from_env() -> OsmiumConfig:
    return build_osmium_config(load_osmium_config_overrides_from_env())
...
def side_offsets(
    ...
    if favored_side == "buy":
        bid_offset -= min(config.max_favored_side_tightening, abs_deviation * config.favored_side_tightening_per_tick)
        ...
        ask_offset -= min(config.max_favored_side_tightening, abs_deviation * config.favored_side_tightening_per_tick)
...
def passive_quotes(
    ...
    # Never rest passive quotes on the wrong side of fixed fair value.
    ...
    inside_bid = None
    inside_ask = None
    if ctx.best_bid_price is not None and ctx.best_ask_price is not None and ctx.best_bid_price < ctx.best_ask_price:
        if ctx.best_bid_price + 1 < ctx.best_ask_price:
            inside_bid = ctx.best_bid_price + 1
        if ctx.best_ask_price - 1 > ctx.best_bid_price:
            inside_ask = ctx.best_ask_price - 1

    # Away from fair, the favored side can lean slightly tighter.
    if abs_deviation > config.moderate_deviation_band:
        if favored_side == "buy" and inside_bid is not None:
            bid_quote = max(bid_quote, inside_bid)
        elif favored_side == "sell" and inside_ask is not None:
            ask_quote = min(ask_quote, inside_ask)

    # In stretched regimes we prioritize fills on the reversion-favored side by
    # joining the current best quote instead of sitting deeper in the book.
    if ctx.best_bid_price is not None and ctx.best_ask_price is not None and ctx.best_bid_price < ctx.best_ask_price:
        if abs_deviation >= config.stretched_deviation_band:
            if favored_side == "buy":
                bid_quote = max(bid_quote, ctx.best_bid_price)
            elif favored_side == "sell":
                ask_quote = min(ask_quote, ctx.best_ask_price)

    # Re-apply the fixed-fair guardrail after any queue-priority adjustments.
    bid_quote = min(bid_quote, config.fair_value - config.fair_value_quote_buffer)
    ask_quote = max(ask_quote, config.fair_value + config.fair_value_quote_buffer)
...
class Trader:
    def __init__(
        ...
        merged_overrides = load_osmium_config_overrides_from_env()
        if config_overrides:
            merged_overrides.update(dict(config_overrides))
        self.config = build_osmium_config(merged_overrides)
