...
class SignalAwarePassiveMarketMakerStrategy(PepperStrategy):
    ...
    def __init__(self, config: SignalAwarePassiveMarketMakerConfig):
        self.config = config

    def build_orders(self, ctx: StrategyContext) -> StrategyResult:
        strategy_limit = effective_strategy_position_limit(
            exchange_limit=ctx.position_limit,
            internal_limit=self.config.internal_position_limit,
        )
        trend_target_inventory = target_inventory_from_ratio(
            strategy_limit=strategy_limit,
            target_inventory_ratio=self.config.trend_target_inventory_ratio,
        )
        signal_extra_inventory = target_inventory_from_ratio(
            strategy_limit=strategy_limit,
            target_inventory_ratio=self.config.signal_extra_inventory_ratio,
        )

        trend_fair_value, trend_slope = compute_trend_fair_value(ctx, self.config)
        features = compute_signal_features(ctx, trend_fair_value, self.config)
        signal_state, filter_reason = evolve_signal_state(
            ctx=ctx,
            features=features,
            config=self.config,
        )

        target_inventory = trend_target_inventory
        if signal_state.active:
            target_inventory = max(
                -strategy_limit,
                min(strategy_limit, trend_target_inventory + signal_extra_inventory),
            )
        ...
        bid_quote, ask_quote = compute_passive_quotes(
            ctx=ctx,
            reservation_price=reservation_price,
            target_inventory=target_inventory,
            signal_active=signal_state.active,
            spread_ticks=features.spread_ticks,
            config=self.config,
        )
        bid_size, ask_size = compute_quote_sizes(
            position=ctx.position,
            strategy_limit=strategy_limit,
            target_inventory=target_inventory,
            signal_active=signal_state.active,
        ...
        if bid_quote is not None and bid_size > 0:
            orders.append(Order(ctx.product, int(bid_quote), int(bid_size)))
        if ask_quote is not None and ask_size > 0:
            orders.append(Order(ctx.product, int(ask_quote), -int(ask_size)))

        return StrategyResult(
            orders=orders,
            state_update={
                "strategy_position_limit": strategy_limit,
                "trend_target_inventory": trend_target_inventory,
                "signal_extra_inventory": signal_extra_inventory,
                "target_inventory": target_inventory,
                "trend_fair_value": round(trend_fair_value, 4) if trend_fair_value is not None else None,
                "trend_slope_per_timestamp": round(trend_slope, 8),
                "fair_value": round(fair_value, 4) if fair_value is not None else None,
                "reservation_price": round(reservation_price, 4) if reservation_price is not None else None,
                "imbalance_shift_ticks": round(imbalance_shift, 4),
                "inventory_shift_ticks": round(inventory_shift, 4),
                "volatility_ema_ticks": round(features.volatility_ema_ticks, 4) if features.volatility_ema_ticks is not None else None,
                "residual": round(features.residual, 4) if features.residual is not None else None,
                "residual_change": round(features.residual_change, 4) if features.residual_change is not None else None,
                "signal_active": signal_state.active,
                "signal_adjustment_ticks": round(signal_state.adjustment_ticks, 4),
                "signal_strength_ticks": round(signal_state.strength_ticks, 4),
                "signal_steps_remaining": signal_state.steps_remaining,
                "signal_activation_residual": round(signal_state.activation_residual, 4)
                if signal_state.activation_residual is not None
                else None,
                "signal_activation_timestamp": signal_state.activation_timestamp,
                "signal_reason": signal_state.reason,
                "spread_ticks": features.spread_ticks,
                "imbalance": round(features.imbalance, 4) if features.imbalance is not None else None,
                "bid_quote": bid_quote,
                "ask_quote": ask_quote,
                "bid_size": bid_size,
                "ask_size": ask_size,
...
STRATEGY_REGISTRY: Dict[str, PepperStrategy] = {
    SignalAwarePassiveMarketMakerStrategy.name: SignalAwarePassiveMarketMakerStrategy(
        PRODUCT_STRATEGY_CONFIGS[INTARIAN_PEPPER_ROOT]
    ),
}
